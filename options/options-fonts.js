...
function renderCustomFonts() {
  ...
  customFonts.forEach((f) => {
    ...
    const tagClass =
      f.source === "google"
        ? "list-item__tag--google"
        : f.source === "upload"
          ? "list-item__tag--upload"
          : "list-item__tag--bundled";
    const tagLabel =
      f.source === "google"
        ? "Google Fonts"
        : f.source === "upload"
    ...
    const tagTitle =
      f.source === "google"
        ? "Loaded from Google Fonts — requires an internet connection"
        : f.source === "upload"
          ? "Font file uploaded from your device — stored locally in this browser"
...
async function handleFontFile(file) {
  ...
  try {
    ...
    const saved = await FontStorage.saveCustomFont(
      fontName,
      dataUrl,
      formatMap[ext] || ext,
    );
...
async function handleGoogleFont(input) {
  ...
  try {
    ...
    if (!res.ok)
      throw new Error(`Google Fonts returned an error (${res.status})`);
    const css = await res.text();
    if (!css.includes("@font-face")) {
      throw new Error(
        `"${fontName}" was not found on Google Fonts — check the spelling`,
      );
    }
