...
// @ts-nocheck — depends on globals from all options-*.js modules and shared scripts
...
function bindEvents() {
  ...
  document.getElementById('google-font-input').addEventListener('keydown', e => {
    if (e.key === 'Enter') handleGoogleFont(document.getElementById('google-font-input').value);
  });

  // Custom font list — keyboard rename (Enter on editable span)
  document.getElementById('custom-fonts-list').addEventListener('keydown', e => {
  ...
  document.getElementById('custom-fonts-list').addEventListener('click', async e => {
  ...
  async function addSite() {
    const raw = document.getElementById('site-input').value.trim().toLowerCase();
    if (!raw) return;
    // Strip protocol/path
